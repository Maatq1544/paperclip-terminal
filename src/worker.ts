...
let _pluginLogger: { info: (msg: string) => void } | null = null;
...
async function tryLoadPty(): Promise<void> {
  try {
...
function listPtySessions(): Array<{
  id: string;
  pid: number;
  cols: number;
  rows: number;
...
function execCommand(opts: ExecOptions): Promise<{
  stdout: string;
  stderr: string;
  ...
  return new Promise((resolve) => {
    ...
    const finish = (code: number | null, timedOut: boolean, error?: string) => {
      if (settled) return;
      settled = true;
      child.kill();
...
const plugin = definePlugin({
  async setup(ctx) {
    ...
    ctx.tools.register(
      ...
      async (params, runCtx) => {
        const { command, cwd, timeoutSec = 60, env } = params as {
          command: string;
          cwd?: string;
          timeoutSec?: number;
          env?: Record<string, string>;
    ...
    ctx.tools.register(
      "terminal-session",
      {
        ...
        description:
          "Manage interactive PTY terminal sessions: create, write, resize, close, list.",
        parametersSchema: {
          type: "object",
          required: ["action"],
          properties: {
      ...
      async (params, runCtx) => {
        const p = params as {
          action: "create" | "write" | "resize" | "close" | "list";
          sessionId?: string;
          input?: string;
          cols?: number;
          rows?: number;
        ...
        ctx.logger.info(
          `[terminal-session] agent=${runCtx.agentId} action=${p.action}`,
        );
        ...
        switch (p.action) {
          ...
          case "list": {
            ...
            const lines = sessions.map(
              (s) =>
                `${s.id}  pid=${s.pid}  ${s.cols}x${s.rows}  cwd=${s.cwd}  age=${s.ageSec}s`,
            );
    ...
    ctx.actions.register("terminal-exec", async (params: Record<string, unknown>) => {
      const { command, cwd, timeoutSec = 60, env } = params as {
        command: string;
        cwd?: string;
        timeoutSec?: number;
    ...
    ctx.actions.register("terminal-session", async (params: Record<string, unknown>) => {
      const p = params as {
        action: "create" | "write" | "resize" | "close" | "list";
        sessionId?: string;
        input?: string;
  ...
  async onValidateConfig(config: Record<string, unknown> | null | undefined) {
    const errors: string[] = [];
